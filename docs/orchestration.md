...
Compound CLI flows (`/review`, `/review-fix`, `/test`) are **not** graph nodes — they call `invoke("plan")` then `invoke("execute")` via [`agents/workflows.py`](../src/tinylocalcoder/agents/workflows.py). See [`Architecture.md`](Architecture.md) and [`use-cases.md`](use-cases.md).
...
Supporting (not LangGraph agents): `CommandRunner` + `ApprovalGate` (shell), `WorkspaceMemory` (files/todos), workflow helpers (`review` / `review-fix` / `test` prompts).
...
| `Pipeline.invoke_workflow("test", prompt)` | Plan (fixed prompt) then execute |
